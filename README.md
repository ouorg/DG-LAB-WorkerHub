...
工作流会先严格校验 `package.json` 与 lockfile（包含重复 key 检查），再使用 lockfile 安装依赖，校验 Actions workflow 结构，并运行单元测试、TypeScript 检查，然后只调用一次 `scripts/deploy-worker.sh`。统一部署脚本严格按顺序执行：
...
3. 执行 `wrangler d1 migrations apply <D1_DATABASE_NAME> --remote`（默认数据库名称为 `dg-lab-worker-hub`）。首次部署时会创建 schema，并通过 migration 写入默认 settings 和管理员；后续重复部署保持幂等。
...
Hub 会拒绝超过 1950 字符的 JSON 消息；单次波形数组最多包含 100 条 8 字节 HEX 数据。`clientMsg.time` 默认为 5 秒，允许范围为 1 到 60 秒。APP 只能绑定二维码 URL 中指定的控制端。同一 APP 通道的新波形会替换旧的重复发送任务；清空通道或任一配对端断开时，对应重复发送任务会立即停止。可选 Worker 变量 `SOCKET_PULSE_INTERVAL_MS` 用于调整波形重复发送间隔，`HEARTBEAT_INTERVAL` 用于调整心跳间隔。
